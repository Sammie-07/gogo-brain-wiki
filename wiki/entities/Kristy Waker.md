---
type: entity
tags: [person, trainer, community, real-estate]
sources: ["[[Securing Your Domain]]", "[[Creating and Customizing Your YouTube Channel (Kristy Waker)]]", "[[Setting Up Your Google Business Profile for Success (Kristy Waker)]]", "[[Brand Consistency Across Social Media Platforms (Kristy Waker)]]", "[[Effective Posting Schedule for Real Estate Leads (Kristy Waker)]]", "[[Create Professional Instagram Account (Kristy Waker)]]", "[[Domain Forwarding to Your Real Estate Website (Kristy Waker)]]", "[[Creating 3-Way Calls with Calendar — GoHighLevel Setup (Kristy Waker)]]", "[[Adding Your Email Signature to Gmail (Kristy Waker, GSMC)]]", "[[Creating an Email Signature Using Canva (Ellen, GSMC)]]", "[[VA Pay Tutorial (Kristy Waker)]]", "[[Managing a High Performing Team — Gogo Community Call]]", "[[Gogo Bethke on Expert Mentors Live — State of Business (John Kitchens)]]", "[[Mastering Your Numbers — Leads, Time & Revenue]]", "[[Your Bookmarks Cheat Sheet (Kristy Waker and Rafael Mino)]]"]
updated: 2026-08-06
---

# Kristy Waker

[[Gogo Bethke]]'s **first hire** and **Director of Operations** (14+ years) — Gogo's "left-hand girl" (she's left-handed), the only US-based member of the team, and also a presenter in the [[GoGet'Em Community]].

> **Canonical name:** Her name is always spelled **Kristy** (**Kristy Waker**) — never any other spelling.

## Role on Team Gogo

- **First hire** — Gogo hired her ~11–14 years ago, *before* she could truly afford the position. This is Gogo's signature "hire before you can afford it" story: the hire creates the capacity, the capacity creates the income.
- **Director of Operations** — finances, bookkeeping, contracts, NDA reviews, tax support, the technology stack, and all hiring processes. Nicknamed the "TinGogo Mama."
- **Only US-based team member.** High **C** (DISC) personality — catches the details Gogo misses.
- **Delegated hiring authority** — she (with Alejandra) hires for her own department; whoever trains the new hire selects the new hire.
- **Money operations** — runs the monthly numbers meeting with Gogo (~1 hr), compiles the VA master timesheet, processes VA pay (Veem), categorizes QuickBooks transactions, and cross-checks affiliate payouts against the bank account.
- Spent ~6 months building out Gogo's eight [[GoHighLevel]] buckets (one sub-account per business).
- Holds the buffer/orientation call with each new VA before Day 1 to explain Gogo's direct personality.

## Known For (GoGet'Em Community)

- Live walkthrough of domain purchasing on GoDaddy (GSMC 2026)
- YouTube channel creation and customization walkthrough (GSMC 2026)
- Facebook Business Page setup and username configuration (GSMC 2026)
- Business email (Google Workspace) setup walkthrough (GSMC 2026)
- Google Business Profile setup walkthrough (GSMC 2026)
- Brand consistency and cross-linking wrap-up (GSMC 2026)
- Weekly posting schedule + BoldTrail IDX squeeze page build walkthrough (GSMC 2026)
- Instagram professional account creation from phone (GSMC 2026)
- Domain forwarding (GoDaddy → BoldTrail/KVCore with masking) (GSMC 2026)
- GoHighLevel three-way call calendar setup (GSMC 2026)
- Gmail email signature setup + WiseStamp introduction (GSMC 2026)
- Personal brand: Kristy Waker (kristywaker.com)
- Business brand: Kristy's Real Estate Team

## Related

- [[Gogo Bethke]]
- [[GoGet'Em Community]]
- [[Personal Branding]]
- [[Real Estate Teams]]
- [[Productivity]]
